...
`netlify.toml` forces `https://www` and `http://` apex traffic to `https://inchiscm.com/:splat` in one 301 hop (`force = true`). Netlify always upgrades `http://www` to `https://www` on the same host first (HSTS preload), then the www rule sends traffic to the apex. Trailing slashes on existing files collapse via Pretty URLs, not a path-level `/*/` 301 splat — Netlify treats `/*/` as `/*` and 301s unknown paths onto themselves. Path-level 301s (length-unit synonyms, hub aliases, unreduced-eighth fraction aliases, published numeric inch 404 aliases, and published height feet/foot wording aliases) are generated after `next build` into `out/_redirects` by `scripts/generate-inch-alias-redirects.mjs`. They must sit above the terminal `/* /404.html 404` because Netlify processes `_redirects` before `netlify.toml`. Do not add splat/`/:value` 301 generators for unpublished numbers, do not index unreduced 2/8, 4/8, or 6/8 fraction URLs, and do not index height wording aliases such as `/5-feet-7-inches-in-cm`.
