...
Interior URLs stay slashless (`trailingSlash: false` plus Netlify Pretty URLs). Do not add a path-level `/*` / `/*/` splat: Netlify matches trailing slashes optionally, so that rule 301-loops unmatched paths onto themselves. Host, hub, and length-unit synonym 301s stay in `netlify.toml`. Published numeric inch 404 aliases (`/{n}-inches-to-cm` → canonical `/{n}-inch(es)-in-cm`, plus singular and `*-in-centimeters` equivalents) are generated at build time from the same `seo-page-policy.json` inventory used for SSG and the sitemap, then written to `out/_redirects`. Unpublished numbers such as `/999999-inches-to-cm` are omitted so they stay 404.
