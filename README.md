...
Interior URLs stay slashless (`trailingSlash: false` plus a catch-all `/*/` → `/:splat` rule). Host, hub, and length-unit synonym 301s stay in `netlify.toml`. Published numeric inch 404 aliases (`/{n}-inches-to-cm` → canonical `/{n}-inch(es)-in-cm`, plus singular and `*-in-centimeters` equivalents) are generated at build time from the same `seo-page-policy.json` inventory used for SSG and the sitemap, then written to `out/_redirects`. Unpublished numbers are omitted so they stay 404.
