...
10. Existing pages must not be deleted, redirected, noindexed, removed from the sitemap, canonicalized to another page, or replaced by a generic hub as a routine tactic. Current URLs must stay live, indexable, self-canonical, internally accessible, and useful unless the owner explicitly authorizes a synonym consolidation. Owner-authorized length-unit synonym 301s are listed in `src/data/page-registry/unit-pair-synonyms.json` and must remain one-hop redirects to the stronger canonical. Do not mass-redirect live numeric `*-inches-in-cm`, `*-cm-in-inches`, or height `F-I-in-cm` canonicals. Owner-authorized 404 aliases of already published inch pages (`*-to-cm`, `*-in-centimeters`) may 301 to those canonicals as a closed set generated from the published inventory; unpublished numbers must stay 404. Improve weak pages instead of removing them.
